...
\newcommand{\Fpotparamcomp}{
  \begin{figure*}
    ...
    In the left panel we plot $V_{,\chi\chi}(\phi,0)$, the effective mass squared of the $\chi$ field along the $\chi=0$ centre line of the instability. % discuss the shape of the profile and how $\chi_w$, $\lambda_\chi$, and $\phi_w$ relate.
    % Mention that effective mass and $\phi_w$ are both increased logrithmically from (a) to (f)

    In the right panel we plot transverse sections of the potential at $\phi=\phi_w$, the point of minimum $V_{,\chi\chi}(\phi,0)$.
    % Mention that these are offset to lie on (0,0)
...
\textcolor{red}{[Place holder figure for $u\Delta V$ for the values of $u$ used in Fig. \ref{fig:zetasliceu}. Show the transverse section $u\Delta V(\phi_p,\chi)$ and $m^2_\mathrm{eff}(\phi,\chi=0)$.]}}
