...
\textcolor{red}{[By making the FRW ansatz we are neglecting the the role of metric fluctuations. In particular this ansatz leads to us dropping a term in the equations of motion for the fields.]}
%\marginpar{
...
%Setting the initial conditions is done in three parts. The homogeneous parts of the fields and momenta are set on the attractor solution of the homogeneous system. Fluctuations in the fields and momenta are set in Fourier space, each mode being drawn from a multivariate Gaussian distribution matching the symmetric part of the fluctuation correlation matrix as determined by linear theory. Initial conditions for $\Pi^a$ are set so that, given the particular realization of the fluctuations on the lattice, the Hubble parameter obeys a lattice averaged version of the energy constraint. 
