...
Discretizing this action on a cubic lattice of side length $l$ with $N$ equally space points along each dimension leads to the equations of motion
\begin{align}
...
\marginpar{
  Need to mention the dimensionless units of the code and give details in the appendix.
}
\marginpar{
  Should probably make a note here that for brevity the lattice indicies $i,j$ and subscript ${}_\lat$ will be implied throughout.
}
...
\marginpar{
  Here put in some stuff about setting field initial conditions using gaussian random variables that match the (cross-)spectra (or 2-point) calculated from perturbation theory.
  And $\Pi^a$ being set to match the enegery constraint as a mean on the lattice.
  Introduce $\phi$ and $\chi$ as scalar fields and say that at the background level inflation is driven by $\phi$ with $\chi$ in the bottom of the trough.
}
