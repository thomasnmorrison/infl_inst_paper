...
%It will prove useful to write the potential as a sum of two terms $V = V_0 + \Delta V$.
%This split can be though of as starting with a baseline system and applying a stimulus by deforming the potential, $V_0$ being the baseline and $\Delta V$ being the stimulus.
...
\marginpar{
  Mention somewhere that by convension we take $\dot{\phi}<0$, ie inlation proceeds towards smaller values of $\phi$. 
}
